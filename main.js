...
if (process.platform == "win32") { console.log("\x1b[42m", "OK", "\x1b[0m" + " Running on Windows (win32)"); }
else { console.warn('\x1b[43m \x1b[30m', 'WARN', '\x1b[0m', ' Dragoncord is running.. Not on Windows (win32). Dragoncord was not be tested on you platform (' + process.platform + ')'); }
...
app.whenReady().then(() => {
	...
	ipcMain.handle(
		...
		(event, opts) => {
			console.log("[ipcMain.handle] DESKTOP_CAPTURER_GET_SOURCES")
			desktopCapturer.getSources(opts);
		});

	ipcMain.on("restart-app", (event, message) => {
		console.log("[ipcMain] restart-app")
		app.quit();
		app.relaunch();
	});

	ipcMain.on('main-process-info', (event, arg) => {
		console.log("[ipcMain] main-process-info")
		switch (arg) {
	...
	ipcMain.on('main-process-utils', (event, arg) => {
		console.log("[ipcMain] main-process-utils")
		switch (arg) {
			case "dialog":
